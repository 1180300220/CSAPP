...
{
    //对比输入字符串，根据字符串内容决定操作。
    //如果输入的字符串是jobs,直接调用listjob；由于是内建指令，返回值为1
    if(!strcmp(argv[0],"jobs")){
        listjobs(jobs);
        return 1;
    }
    //如果输入的是bg或fg，调用do_bgfg让其自动处理；由于是内建指令，返回值为1
    else if(!strcmp(argv[0],"bg")|| !strcmp(argv[0],"fg")){
        //将字符串作为参数传入do_bgfg，函数内部会自动判断
        do_bgfg(argv);
        return 1;
    }
    //如果输入的是quit，直接退出。
...
void waitfg(pid_t pid)
{
    //fgpid得到前台正在运行的进程号，如果传入的进程正在运行，则调用sleep等待前台进程运行完成
...
void sigchld_handler(int sig) 
{
    pid_t pid;
    int status;
    //判断等待集合中是否有进程被终止或停止,WUNTRACED表示有子进程停止，WNOHANG表示无子进程正在执行。
